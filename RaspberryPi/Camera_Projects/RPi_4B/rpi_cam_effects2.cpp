// Raspberry Pi Camera with Game Of Life Effects Test 1
// Maxwell Bakalos 11/18/2024
...
int main(int argc, char** argv ) {
    ...
    std::string pipeline = "libcamerasrc ! video/x-raw, width=64, height=64, framerate=15/1 ! videoconvert ! appsink";
    cout << "OK" << endl;
    ...
    cv::Mat frame;
    cap >> frame;  // Capture 1st frame from the webcam (to calculate frame size)
    if (frame.empty()) {
    ...
    if (option_valid) {
        ...
        while (true) {
            ...
            if (! im_out.empty()) {
                ...
                cv::resizeWindow("Subplot: OG Image, Convolved Image", subplot.cols *6 , subplot.rows *6); // Set the window to its actual size

...
int main(int argc, char** argv ) {
    ...
    // Load Image
    // string imPath = "bees_flower_640x360.png"; 
    ...
    if (blur_valid) {
        ...
        std::vector<cv::Mat> channels1(3, cv::Mat(dims[1], dims[0], CV_32F));
        std::vector<cv::Mat> channels2(3, cv::Mat(dims[1], dims[0], CV_32F));

        cout << "Test 1" << endl; // **************************************

        // Split the image into B, G, R channels
        cv::Mat im_float;
        im.convertTo(im_float, CV_64F, 1.0 / 255.0);
        // im.convertTo(im_float, CV_32F, 1.0 / 255.0); // I want to use 32F but it turns the output black & white!
        cv::split(im_float, channels1);

        float min_bound = static_cast<float>(0.2);
        float max_bound = static_cast<float>(0.45);
        float bound_diff = max_bound - min_bound;
        float shx = min_bound, shy = static_cast<float>(0.5), scale = static_cast<float>(0.5);
        float pi_f = static_cast<float>(3.14159265358979323846);
        ...
        for (int rgb = 0; rgb < 3; rgb++) {
            ...
            if (x.type() != CV_32F) {
                cout << "x is not CV_32F!!! Converting now." << endl;
                x.convertTo(x, CV_32F);
            } else {
                cout << "x is CV_32F" << endl;
            }
            ...
            cout << "X Minimum value: " << minVal << " at location " << minLoc << endl;
            cout << "X Maximum value: " << maxVal << " at location " << maxLoc << endl;
            cv::Scalar meanVal = cv::mean(x);
            cout << "X Average value: " << meanVal[0] << endl;
            ...
            cout << "LTM Minimum value: " << minVal << " at location " << minLoc << endl;
            cout << "LTM Maximum value: " << maxVal << " at location " << maxLoc << endl;
            meanVal = cv::mean(lessThanMask);
            cout << "LTM Average value: " << meanVal[0] << endl;

            // Minimum, Maximum, & Average check 1
            // double minVal, maxVal;
            // cv::Point minLoc, maxLoc;
            cv::minMaxLoc(y, &minVal, &maxVal, &minLoc, &maxLoc);
            cout << "Y Minimum value: " << minVal << " at location " << minLoc << endl;
            cout << "Y Maximum value: " << maxVal << " at location " << maxLoc << endl;
            meanVal = cv::mean(y);
            cout << "Y Average value: " << meanVal[0] << endl;
